[app]

# ------------------------------------------------------------------------
# 基本情報
# ------------------------------------------------------------------------
title = ESDUCT
package.name = esduct
package.domain = jp.jl7khn

source.dir = .
source.include_exts = py,png,jpg,kv,atlas,json,txt,html,css,js
source.exclude_dirs = .github,.git,.buildozer,bin,__pycache__,recipes
source.exclude_patterns = README.md,*.md,.gitignore

version = 1.0

# ------------------------------------------------------------------------
# Python依存パッケージ
# ------------------------------------------------------------------------
# numpy: 2.3.2にはunique.cppで<unordered_map>のincludeが
# 抜けているビルドバグがあるため、修正済みの2.3.3以降を明示指定する
requirements = python3,kivy==2.3.1,requests,beautifulsoup4,pillow,numpy==2.3.3,pandas,plyer,pyjnius,certifi,charset-normalizer,idna,urllib3,six,python-dateutil,pytz

orientation = portrait
fullscreen = 0

# ------------------------------------------------------------------------
# Android固有設定
# ------------------------------------------------------------------------
android.permissions = INTERNET,ACCESS_FINE_LOCATION,ACCESS_COARSE_LOCATION,WAKE_LOCK,READ_EXTERNAL_STORAGE,WRITE_EXTERNAL_STORAGE
android.archs = arm64-v8a
android.minapi = 24
android.api = 33
android.ndk = 25b

android.accept_sdk_license = True
android.enable_androidx = True
android.allow_backup = True

# ------------------------------------------------------------------------
# python-for-android設定
# ------------------------------------------------------------------------
p4a.bootstrap = sdl2
p4a.branch = master
p4a.local_recipes = recipes

# ------------------------------------------------------------------------
# ログ設定
# ------------------------------------------------------------------------
log_level = 2
warn_on_root = 1

[buildozer]
log_level = 2
warn_on_root = 1
